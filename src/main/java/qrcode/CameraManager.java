...
public final class CameraManager {
	 ...
	 */
	public static void init(Context context,CaptureView captureView) {
		if (cameraManager == null) {
			cameraManager = new CameraManager(context,captureView);
		}
	}
	...
	private CameraManager(Context context,CaptureView captureView) {

		this.context = context;
		this.captureView=captureView;
		this.configManager = new CameraConfigurationManager(this.context,this.captureView);
...
//			camera.setParameters(params);
	...
	public Rect getFramingRect() {
		...
		if (framingRect == null) {
			...
			int min, leftOffset, topOffset, width, height;
			xCenter =screenResolution.x / 2;
			yCenter = screenResolution.y *2/ 5;
			min = Math.min(xCenter, yCenter);
			int radius = min * 4 / 7;
			leftOffset = xCenter - radius;
			topOffset = yCenter - radius;
			width = radius * 2;
			height = width;
...
//			rect.top=captureView.getRight()-xCenter-framRect.width()/2;
//			rect.bottom=rect.top+framRect.width();
//			
//			rect.left=0;
//			rect.top=0;
//			rect.bottom=captureView.getRight();
//			rect.right=2000;
//			
...
//			System.out.println(rect.bottom);
	...
	public PlanarYUVLuminanceSource buildLuminanceSource(byte[] data,
		...
		case ImageFormat.NV21: // old: case PixelFormat.YCbCr_420_SP:
			// This format has never been seen in the wild, but is compatible as
			// we only care
			// about the Y channel, so allow it.
		case ImageFormat.YUY2: // old: PixelFormat.YCbCr_422_SP:
