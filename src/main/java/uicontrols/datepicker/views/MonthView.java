...
import uicontrols.datepicker.bizs.CalendarBiz;
import uicontrols.datepicker.entities.BGCircle;
...
public class MonthView extends View implements ValueAnimator.AnimatorUpdateListener {
    ...
    private int colorBooking = 0xFFFDEBB0;
    private int colorFont = 0xffc9ae50;
    private int colorChoice = 0xffc9ae50;
    ...
    private Map<String, BGCircle> circlesBooking = new HashMap<>();
    private List<String> dateSelected = new ArrayList<>();
    private List<String> dateBooking = new ArrayList<>();
    private boolean choiceInBooking=false;
     ...
     * @param colorBooking ...
     */
    public void setColorBooking(int colorBooking) {
        this.colorBooking = colorBooking;
    }

    public void setDateSelected(List<String> dateSelected) {
        this.dateSelected = dateSelected;
    }

    public void setDateBooking(List<String> dateBooking) {
        this.dateBooking = dateBooking;
    }

    public void setChoiceInBooking(boolean choiceInBooking) {
        this.choiceInBooking = choiceInBooking;
    }
    ...
    private BGCircle createCircle(float x, float y, int color) {
    ...
    private void defineContainRegion(int x, int y) {
        for (int i = 0; i < mRegion.length; i++) {
            for (int j = 0; j < mRegion[i].length; j++) {
                ...
                // Log.e("ddd", currentYear + "-" + currentMonth + "-" + mCalendarBiz.getGregorianCreated().get(index)[i][j]);
                if (region.contains(x, y)) {
                    ...
                    if (choiceInBooking)
                    {
                        if (!dateBooking.contains(date)) return;
                    }
                    if (dateSelected.contains(date)) {
                        ...
                        BGCircle circle = createCircle(region.getBounds().centerX() + index * sizeBase, region.getBounds().centerY(), colorChoice);
    ...
    public void loadRegion() {
        for (int i = 0; i < mRegion.length; i++) {
            for (int j = 0; j < mRegion[i].length; j++) {
                ...
                if (dateBooking.contains(date)) {
                    BGCircle circle = createCircle(region.getBounds().centerX() + index * sizeBase, region.getBounds().centerY(), colorBooking);
                    ValueAnimator animScale4 = ObjectAnimator.ofInt(circle, "radius", animZoomOut2, circleRadius);
                    animScale4.setDuration(1);
                    animScale4.setInterpolator(new AccelerateInterpolator());
                    animScale4.addUpdateListener(this);
                    AnimatorSet animSet = new AnimatorSet();
                    animSet.playSequentially(animScale4);
                    animSet.start();
                    circlesBooking.put(date, circle);
                }
                if (dateSelected.contains(date)) {
                    List<Region> regions = calendarRegion.get(index);
                    if (!regions.contains(region))
    ...
    private void drawCircle(Canvas canvas) {
        for (String s : circlesBooking.keySet()) {
            BGCircle circle = circlesBooking.get(s);
    ...
    private void drawBGCircle(Canvas canvas, BGCircle circle) {
        ...
        circle.getShape().getShape().resize(circle.getRadius() * 0.95f, circle.getRadius() * 0.95f);
    ...
    private void drawMonth(Canvas canvas, float offsetX, int year, int month) {
        ...
        for (int i = 0; i < gregorianCurrent.length; i++) {
            for (int j = 0; j < gregorianCurrent[i].length; j++) {
                ...
                if (j == 0 || j == gregorianCurrent[i].length - 1)
                    mTextPaint.setColor(colorFont);
                else
